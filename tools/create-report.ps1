...
$body += P 'La clase Calculadora implementa suma y resta con detección de desbordamiento. La clase App expone /health y /calculate. Se eligió JUnit 5 para las pruebas; Selenium no aporta valor en esta aplicación sin interfaz gráfica. Repositorio publicado: https://github.com/a-hernandez88/iplacex. Las ramas main y feature/primer-test son visibles en GitHub.'
...
$body += P 'Evidencia local: evidence/verify-local.log registra BUILD SUCCESS; 3 pruebas unitarias y 3 pruebas de integración, sin errores ni fallos. Evidencia en GitHub: https://github.com/a-hernandez88/iplacex/actions/runs/35622120211 terminó exitosamente, con build, unit-tests, integration-tests y acceptance-deploy-rollback en estado success. El detalle y los enlaces de jobs se encuentran en evidence/github-actions.txt.'
...
$body += P 'Desde examen-final: mvn -B test ejecuta unitarias; mvn -B verify ejecuta build, unitarias e integración; bash deployment/deploy-demo.sh ejecuta aceptación, despliegue y rollback. El repositorio publicado incluye pom.xml, código fuente, workflow, scripts, README.md, logs locales y evidencia de GitHub Actions.'
$body += P 'Justificación frente a la pauta' 'heading'
$body += P 'Configuración y funcionalidad: Git, Maven, JUnit y pipeline comprobados localmente y en GitHub Actions. Cumplimiento: las tres actividades disponen de archivos ejecutables y evidencias públicas del run. Código limpio: separación entre aplicación, pruebas y despliegue, nombres descriptivos y flujo de ramas definido. Presentación: README, informe, logs y enlaces verificables.'
